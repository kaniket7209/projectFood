...
def sign_up():
    ...
    if request.method == "POST":
        ...
        mydb.reconnect()
        cur = mydb.cursor()
        cur.execute("SELECT COUNT(mobile_no) FROM registered_users_info WHERE mobile_no = %s",[phone_no])
        ...
        res = requests.get("http://localhost:8000/sendOTP?number={}&otp={}".format(phone_no,otp))
...
def send_otp_again():
    from functions import otpgen  
    otp = otpgen()
    res = requests.get("http://localhost:8000/sendOTP?number={}&otp={}".format(session['user_phone_no'],otp))
    print(res.status_code)
    session['otp'] = otp
    return redirect(url_for('verification'))
    ...
    if request.method == "POST":
        try:
            ...
            if otpEntered == session['otp']:
                ...
                mydb.reconnect()
                cur = mydb.cursor()
                cur.execute("SELECT COUNT(mobile_no) FROM registered_users_info WHERE mobile_no = %s",[session['user_phone_no']])
                count = cur.fetchall()[0][0]
                # print(count,"--")
                if count >=1: # for registered user  login verification
                    ...
                    val=(session['user_phone_no'],session['email'],session['password'])
...
def sign_in():
    if request.method == "POST":
        try:
            ...
            mydb.reconnect()
            cur = mydb.cursor()
            cur.execute("SELECT mobile_no, password FROM registered_users_info WHERE mobile_no = %s",[phone_no])
            result = cur.fetchall()
...
def personal_details():
    if request.method == "POST":
        ...
        if image:
            print(image)
            image.save(os.path.join(saveImageDirPath, secure_filename(image.filename)))
        print(request.form)
        print(storeName)
